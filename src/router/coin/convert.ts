const crypto: { [key: string]: string } = {
  eos: 'eos',
  trx: 'tron',
  '42': '42-coin',
  '404': '404-coin',
  '611': 'sixeleven',
  '888': 'octocoin',
  '1337': '1337',
  bitcoin: 'bitcoin',
  btc: 'bitcoin',
  etc: 'ethereum-classic',
  ethereum: 'ethereum',
  eth: 'ethereum',
  steem: 'steem',
  ripple: 'ripple',
  xrp: 'ripple',
  litecoin: 'litecoin',
  ltc: 'litecoin',
  'the-dao': 'the-dao',
  dao: 'the-dao',
  dash: 'dash',
  nem: 'nem',
  xem: 'nem',
  maidsafecoin: 'maidsafecoin',
  maid: 'maidsafecoin',
  emercoin: 'emercoin',
  emc: 'emercoin',
  lisk: 'lisk',
  lsk: 'lisk',
  dogecoin: 'dogecoin',
  doge: 'dogecoin',
  nxt: 'nxt',
  synereo: 'synereo',
  amp: 'synereo',
  monero: 'monero',
  xmr: 'monero',
  digixdao: 'digixdao',
  dgd: 'digixdao',
  waves: 'waves',
  factom: 'factom',
  fct: 'factom',
  bitshares: 'bitshares',
  bts: 'bitshares',
  siacoin: 'siacoin',
  sc: 'siacoin',
  stellar: 'stellar',
  xlm: 'stellar',
  'bytecoin-bcn': 'bytecoin-bcn',
  bcn: 'bytecoin-bcn',
  peercoin: 'peercoin',
  ppc: 'peercoin',
  counterparty: 'counterparty',
  xcp: 'counterparty',
  gamecredits: 'gamecredits',
  game: 'gamecredits',
  tether: 'tether',
  usdt: 'tether',
  'storjcoin-x': 'storjcoin-x',
  sjcx: 'storjcoin-x',
  namecoin: 'namecoin',
  nmc: 'namecoin',
  europecoin: 'europecoin',
  erc: 'europecoin',
  nautiluscoin: 'nautiluscoin',
  naut: 'nautiluscoin',
  ybcoin: 'ybcoin',
  ybc: 'ybcoin',
  'agoras-tokens': 'agoras-tokens',
  agrs: 'agoras-tokens',
  syscoin: 'syscoin',
  sys: 'syscoin',
  sarcoin: 'sarcoin',
  sar: 'sarcoin',
  bitcrystals: 'bitcrystals',
  bcy: 'bitcrystals',
  decred: 'decred',
  dcr: 'decred',
  xaurum: 'xaurum',
  xau: 'xaucoin',
  solarcoin: 'solarcoin',
  slr: 'solarcoin',
  rubycoin: 'rubycoin',
  rby: 'rubycoin',
  fedoracoin: 'fedoracoin',
  tips: 'fedoracoin',
  digibyte: 'digibyte',
  dgb: 'digibyte',
  vpncoin: 'vpncoin',
  vpn: 'vpncoin',
  yocoin: 'yocoin',
  yoc: 'yocoin',
  blackcoin: 'blackcoin',
  blk: 'blackcoin',
  iocoin: 'iocoin',
  ioc: 'iocoin',
  voxels: 'voxels',
  vox: 'voxels',
  feathercoin: 'feathercoin',
  ftc: 'feathercoin',
  gridcoin: 'gridcoin',
  grc: 'gridcoin',
  vericoin: 'vericoin',
  vrc: 'vericoin',
  'supernet-unity': 'supernet-unity',
  unity: 'supernet-unity',
  bitcoindark: 'bitcoindark',
  btcd: 'bitcoindark',
  scotcoin: 'scotcoin',
  scot: 'scotcoin',
  hitcoin: 'hitcoin',
  htc: 'hitcoin',
  monacoin: 'monacoin',
  mona: 'monacoin',
  mintcoin: 'mintcoin',
  mint: 'mintcoin',
  vcash: 'vcash',
  xvc: 'vcash',
  'library-credit': 'library-credit',
  lbc: 'library-credit',
  shadowcoin: 'shadowcoin',
  sdc: 'shadowcoin',
  dnotes: 'dnotes',
  note: 'dnotes',
  omni: 'omni',
  earthcoin: 'earthcoin',
  eac: 'earthcoin',
  breakout: 'breakout',
  brk: 'breakout',
  coinshield: 'coinshield',
  niro: 'coinshield',
  primecoin: 'primecoin',
  xpm: 'primecoin',
  expanse: 'expanse',
  exp: 'expanse',
  clams: 'clams',
  clam: 'clams',
  auroracoin: 'auroracoin',
  aur: 'auroracoin',
  quark: 'quark',
  qrk: 'quark',
  reddcoin: 'reddcoin',
  rdd: 'reddcoin',
  radium: 'radium',
  rads: 'radium',
  novacoin: 'novacoin',
  nvc: 'novacoin',
  rise: 'rise',
  'bitshares-music': 'bitshares-music',
  muse: 'bitshares-music',
  startcoin: 'startcoin',
  start: 'startcoin',
  worldcoin: 'worldcoin',
  wdc: 'worldcoin',
  faircoin: 'faircoin',
  fair: 'faircoin',
  blocknet: 'blocknet',
  block: 'blocknet',
  vertcoin: 'vertcoin',
  vtc: 'vertcoin',
  'global-currency-reserve': 'global-currency-reserve',
  gcr: 'global-currency-reserve',
  darknote: 'darknote',
  xdn: 'darknote',
  gulden: 'gulden',
  nlg: 'gulden',
  qora: 'qora',
  'skynet-asset': 'skynet-asset',
  skynet: 'skynet-asset',
  boostcoin: 'boostcoin',
  bost: 'boostcoin',
  obits: 'obits',
  instantdex: 'instantdex',
  dex: 'instantdex',
  curecoin: 'curecoin',
  cure: 'curecoin',
  fimkrypto: 'fimkrypto',
  fimk: 'fimkrypto',
  swarm: 'swarm',
  megacoin: 'megacoin',
  mec: 'megacoin',
  'nav-coin': 'nav-coin',
  nav: 'nav-coin',
  i0coin: 'i0coin',
  i0c: 'i0coin',
  nubits: 'nubits',
  usnbt: 'nubits',
  bilshares: 'bilshares',
  bils: 'bilshares',
  'hempcoin-hmp': 'hempcoin-hmp',
  hmp: 'hempcoin-hmp',
  jl777hodl: 'jl777hodl',
  jlh: 'jl777hodl',
  bitbay: 'bitbay',
  bay: 'bitbay',
  mmnxt: 'mmnxt',
  'bitshares-pts': 'bitshares-pts',
  pts: 'bitshares-pts',
  crypti: 'crypti',
  xcr: 'crypti',
  xcurrency: 'xcurrency',
  xc: 'xcurrency',
  virtacoin: 'virtacoin',
  vta: 'virtacoin',
  zccoin: 'zccoin',
  zcc: 'zccoin',
  nushares: 'nushares',
  nsr: 'nushares',
  crevacoin: 'crevacoin',
  creva: 'crevacoin',
  florincoin: 'florincoin',
  flo: 'florincoin',
  silkcoin: 'silkcoin',
  silk: 'silkcoin',
  pebblecoin: 'pebblecoin',
  xpb: 'pebblecoin',
  'safe-exchange-coin': 'safe-exchange-coin',
  sec: 'safe-exchange-coin',
  bitswift: 'bitswift',
  swift: 'bitswift',
  diamond: 'diamond',
  dmd: 'diamond',
  digitalcoin: 'digitalcoin',
  dgc: 'digitalcoin',
  unioncoin: 'unioncoin',
  unc: 'uncoin',
  reecoin: 'reecoin',
  ree: 'reecoin',
  infinitecoin: 'infinitecoin',
  ifc: 'infinitecoin',
  zeitcoin: 'zeitcoin',
  zeit: 'zeitcoin',
  applecoin: 'applecoin',
  apc: 'alpacoin',
  ribbitrewards: 'ribbitrewards',
  rbr: 'ribbitrewards',
  neucoin: 'neucoin',
  neu: 'neucoin',
  letitride: 'letitride',
  lir: 'letitride',
  burst: 'burst',
  diem: 'diem',
  anoncoin: 'anoncoin',
  anc: 'anoncoin',
  'wild-beast-block': 'wild-beast-block',
  wbb: 'wild-beast-block',
  zetacoin: 'zetacoin',
  zet: 'zetacoin',
  liquid: 'liquid',
  lqd: 'liquid',
  onecurrency: 'onecurrency',
  onec: 'onecurrency',
  dubaicoin: 'dubaicoin',
  dbic: 'dubaicoin',
  vootcoin: 'vootcoin',
  voot: 'vootcoin',
  evergreencoin: 'evergreencoin',
  egc: 'evergreencoin',
  riecoin: 'riecoin',
  ric: 'riecoin',
  unobtanium: 'unobtanium',
  uno: 'unobtanium',
  cryptojacks: 'cryptojacks',
  cj: 'cryptojacks',
  viacoin: 'viacoin',
  via: 'viacoin',
  '2give': '2give',
  cryptofund: 'cryptofund',
  fund: 'cryptofund',
  gambit: 'gambit',
  gam: 'gambit',
  applebyte: 'applebyte',
  aby: 'applebyte',
  verge: 'verge',
  xvg: 'verge',
  ambercoin: 'ambercoin',
  amber: 'ambercoin',
  trumpcoin: 'trumpcoin',
  trump: 'trumpcoin',
  syndicate: 'syndicate',
  synx: 'syndicate',
  'elcoin-el': 'elcoin-el',
  el: 'elcoin-el',
  cryptonite: 'cryptonite',
  xcn: 'cryptonite',
  'pangea-poker': 'pangea-poker',
  pangea: 'pangea-poker',
  coinomat: 'coinomat',
  cnmt: 'coinomat',
  'colossuscoin-v2': 'colossuscoin-v2',
  cv2: 'colossuscoin-v2',
  tagcoin: 'tagcoin',
  tag: 'tagcoin',
  bitmark: 'bitmark',
  btm: 'bitmark',
  myriadcoin: 'myriadcoin',
  xmy: 'myriadcoin',
  goldcoin: 'goldcoin',
  gld: 'goldcoin',
  securecoin: 'securecoin',
  src: 'securecoin',
  casinocoin: 'casinocoin',
  csc: 'casinocoin',
  okcash: 'okcash',
  ok: 'okcash',
  hicoin: 'hicoin',
  xhi: 'hicoin',
  trustplus: 'trustplus',
  trust: 'trustplus',
  gems: 'gems',
  gemz: 'gems',
  boolberry: 'boolberry',
  bbr: 'boolberry',
  mooncoin: 'mooncoin',
  moon: 'mooncoin',
  salus: 'salus',
  sls: 'salus',
  bluecoin: 'bluecoin',
  blu: 'bluecoin',
  huntercoin: 'huntercoin',
  huc: 'huntercoin',
  potcoin: 'potcoin',
  pot: 'potcoin',
  bitcny: 'bitcny',
  mineum: 'mineum',
  mnm: 'mineum',
  stealthcoin: 'stealthcoin',
  xst: 'stealthcoin',
  groestlcoin: 'groestlcoin',
  grs: 'groestlcoin',
  capricoin: 'capricoin',
  cpc: 'capricoin',
  aeon: 'aeon',
  energycoin: 'energycoin',
  enrg: 'energycoin',
  shift: 'shift',
  einsteinium: 'einsteinium',
  emc2: 'einsteinium',
  audiocoin: 'audiocoin',
  adc: 'audiocoin',
  krypton: 'krypton',
  kr: 'krypton',
  neoscoin: 'neoscoin',
  neos: 'neoscoin',
  spreadcoin: 'spreadcoin',
  spr: 'spreadcoin',
  sync: 'sync',
  fuelcoin: 'fuelcoin',
  fc2: 'fuelcoin',
  noblecoin: 'noblecoin',
  nobl: 'noblecoin',
  maxcoin: 'maxcoin',
  max: 'maxcoin',
  'pandacoin-pnd': 'pandacoin-pnd',
  pnd: 'pandacoin-pnd',
  orbitcoin: 'orbitcoin',
  orb: 'orbitcoin',
  'next-horizon': 'next-horizon',
  hz: 'next-horizon',
  coinmarketscoin: 'coinmarketscoin',
  jbs: 'coinmarketscoin',
  cannabiscoin: 'cannabiscoin',
  cann: 'cannabiscoin',
  sibcoin: 'sibcoin',
  sib: 'sibcoin',
  smileycoin: 'smileycoin',
  smly: 'smileycoin',
  'electronic-gulden': 'electronic-gulden',
  efl: 'electronic-gulden',
  'dt-token': 'dt-token',
  draco: 'dt-token',
  piggycoin: 'piggycoin',
  piggy: 'piggycoin',
  darknet: 'darknet',
  dnet: 'darknet',
  mediterraneancoin: 'mediterraneancoin',
  med: 'mediterraneancoin',
  woodcoin: 'woodcoin',
  log: 'woodcoin',
  noirshares: 'noirshares',
  nrs: 'noirshares',
  memorycoin: 'memorycoin',
  mmc: 'memorycoin',
  foldingcoin: 'foldingcoin',
  fldc: 'foldingcoin',
  netcoin: 'netcoin',
  net: 'netcoin',
  fluttercoin: 'fluttercoin',
  flt: 'fluttercoin',
  pakcoin: 'pakcoin',
  pak: 'pakcoin',
  creditbit: 'creditbit',
  crbit: 'creditbit',
  geocoin: 'geocoin',
  geo: 'geocoin',
  tickets: 'tickets',
  tix: 'tickets',
  mazacoin: 'mazacoin',
  mzc: 'mazacoin',
  'cryptogenic-bullion': 'cryptogenic-bullion',
  cbx: 'cryptogenic-bullion',
  bytecent: 'bytecent',
  byc: 'bytecent',
  'coin2-1': 'coin2-1',
  c2: 'coin2-1',
  bitusd: 'bitusd',
  hyperstake: 'hyperstake',
  hyp: 'hyperstake',
  pinkcoin: 'pinkcoin',
  pink: 'pinkcoin',
  btsr: 'btsr',
  mojocoin: 'mojocoin',
  mojo: 'mojocoin',
  flycoin: 'flycoin',
  fly: 'flycoin',
  ltbcoin: 'ltbcoin',
  ltbc: 'ltbcoin',
  influxcoin: 'influxcoin',
  infx: 'influxcoin',
  maryjane: 'maryjane',
  maryj: 'maryjane',
  rubies: 'rubies',
  rbies: 'rubies',
  magi: 'magi',
  xmg: 'magi',
  joincoin: 'joincoin',
  j: 'joincoin',
  hyper: 'hyper',
  fibre: 'fibre',
  paycoin2: 'paycoin2',
  xpy: 'paycoin2',
  sprouts: 'sprouts',
  sprts: 'sprouts',
  bitbean: 'bitbean',
  bitb: 'bitbean',
  britcoin: 'britcoin',
  brit: 'britcoin',
  teslacoin: 'teslacoin',
  tes: 'teslacoin',
  bitcredits: 'bitcredits',
  bcr: 'bitcredits',
  truckcoin: 'truckcoin',
  trk: 'truckcoin',
  blitzcash: 'blitzcash',
  blitz: 'blitzcash',
  quatloo: 'quatloo',
  qtl: 'quatloo',
  cryptcoin: 'cryptcoin',
  crypt: 'cryptcoin',
  whitecoin: 'whitecoin',
  xwc: 'whitecoin',
  'the-cypherfunks': 'the-cypherfunks',
  funk: 'the-cypherfunks',
  exclusivecoin: 'exclusivecoin',
  excl: 'exclusivecoin',
  hodlcoin: 'hodlcoin',
  hodl: 'hodlcoin',
  yaccoin: 'yaccoin',
  yacc: 'yaccoin',
  transfercoin: 'transfercoin',
  tx: 'transfercoin',
  sexcoin: 'sexcoin',
  sxc: 'sexcoin',
  bitsend: 'bitsend',
  bsd: 'bitsend',
  redcoin: 'redcoin',
  red: 'redcoin',
  parkbyte: 'parkbyte',
  pkb: 'parkbyte',
  fantomcoin: 'fantomcoin',
  fcn: 'fantomcoin',
  aiden: 'aiden',
  adn: 'aiden',
  'bitcoin-plus': 'bitcoin-plus',
  xbc: 'bitcoin-plus',
  adzcoin: 'adzcoin',
  adz: 'adzcoin',
  'cryptographic-anomaly': 'cryptographic-anomaly',
  cga: 'cryptographic-anomaly',
  devcoin: 'devcoin',
  dvc: 'devcoin',
  qibuck: 'qibuck',
  qbk: 'qibuck',
  gapcoin: 'gapcoin',
  gap: 'gapcoin',
  zaif: 'zaif',
  bata: 'bata',
  bat: 'basic-attention-token',
  bta: 'bata',
  hempcoin: 'hempcoin',
  thc: 'hempcoin',
  'deutsche-emark': 'deutsche-emark',
  dem: 'deutsche-emark',
  digicube: 'digicube',
  cube: 'digicube',
  'the-viral-exchange': 'the-viral-exchange',
  tve: 'the-viral-exchange',
  nxttycoin: 'nxttycoin',
  nxtty: 'nxttycoin',
  'ufo-coin': 'ufo-coin',
  ufo: 'ufo-coin',
  titcoin: 'titcoin',
  tit: 'titcoin',
  secretcoin: 'secretcoin',
  scrt: 'secretcoin',
  uro: 'uro',
  terracoin: 'terracoin',
  trc: 'terracoin',
  dimecoin: 'dimecoin',
  dime: 'dimecoin',
  tekcoin: 'tekcoin',
  tek: 'tekcoin',
  'sapience-aifx': 'sapience-aifx',
  xai: 'sapience-aifx',
  synergy: 'synergy',
  snrg: 'synergy',
  asiadigicoin: 'asiadigicoin',
  adcn: 'asiadigicoin',
  bitbtc: 'bitbtc',
  freicoin: 'freicoin',
  frc: 'freicoin',
  monetaverde: 'monetaverde',
  mcn: 'monetaverde',
  dopecoin: 'dopecoin',
  dope: 'dopecoin',
  bbqcoin: 'bbqcoin',
  bqc: 'bbqcoin',
  'spots-spots': 'spots-spots',
  spots: 'spots',
  ziftrcoin: 'ziftrcoin',
  zrc: 'ziftrcoin',
  blakecoin: 'blakecoin',
  blc: 'blakecoin',
  memetic: 'memetic',
  meme: 'memetic',
  korecoin: 'korecoin',
  kore: 'korecoin',
  usde: 'usde',
  bellacoin: 'bellacoin',
  bela: 'bellacoin',
  putincoin: 'putincoin',
  putin: 'putincoin',
  viral: 'viral',
  '1credit': '1credit',
  '1cr': '1credit',
  metalcoin: 'metalcoin',
  metal: 'metalcoin',
  pesetacoin: 'pesetacoin',
  ptc: 'pesetacoin',
  sativacoin: 'sativacoin',
  stv: 'sativacoin',
  sling: 'sling',
  monetaryunit: 'monetaryunit',
  mue: 'monetaryunit',
  supercoin: 'supercoin',
  super: 'supercoin',
  goldblocks: 'goldblocks',
  gb: 'goldblocks',
  moin: 'moin',
  neutron: 'neutron',
  ntrn: 'neutron',
  archcoin: 'archcoin',
  arch: 'archcoin',
  soilcoin: 'soilcoin',
  soil: 'soilcoin',
  chipcoin: 'chipcoin',
  chip: 'chipcoin',
  primechain: 'primechain',
  prime: 'primechain',
  comet: 'comet',
  cmt: 'comet',
  quazarcoin: 'quazarcoin',
  qcn: 'quazarcoin',
  franko: 'franko',
  frk: 'franko',
  'canada-ecoin': 'canada-ecoin',
  cdn: 'canada-ecoin',
  postcoin: 'postcoin',
  post: 'postcoin',
  steps: 'steps',
  'gpu-coin': 'gpu-coin',
  gpu: 'gpu-coin',
  billarycoin: 'billarycoin',
  blry: 'billarycoin',
  francs: 'francs',
  frn: 'francs',
  cryptoescudo: 'cryptoescudo',
  cesc: 'cryptoescudo',
  orangecoin: 'orangecoin',
  oc: 'orangecoin',
  smartcoin: 'smartcoin',
  smc: 'smartcoin',
  bitsilver: 'bitsilver',
  dlisk: 'dlisk',
  bitz: 'bitz',
  'globalboost-y': 'globalboost-y',
  bsty: 'globalboost-y',
  mindcoin: 'mindcoin',
  mnd: 'mindcoin',
  'bios-crypto': 'bios-crypto',
  bios: 'bios-crypto',
  viorcoin: 'viorcoin',
  vior: 'viorcoin',
  goldpieces: 'goldpieces',
  gp: 'goldpieces',
  grantcoin: 'grantcoin',
  grt: 'grantcoin',
  ucoin: 'ucoin',
  u: 'ucoin',
  corgicoin: 'corgicoin',
  corg: 'corgicoin',
  sooncoin: 'sooncoin',
  soon: 'sooncoin',
  paycon: 'paycon',
  con: 'paycon',
  'universal-currency': 'universal-currency',
  unit: 'universal-currency',
  warp: 'warp',
  'prime-xi': 'prime-xi',
  pxi: 'prime-xi',
  '8bit': '8bit',
  debitcoin: 'debitcoin',
  dbtc: 'debitcoin',
  plncoin: 'plncoin',
  plnc: 'plncoin',
  'c-bit': 'c-bit',
  xct: 'c-bit',
  unbreakablecoin: 'unbreakablecoin',
  unb: 'unbreakablecoin',
  kittehcoin: 'kittehcoin',
  meow: 'kittehcoin',
  apexcoin: 'apexcoin',
  apex: 'apexcoin',
  antibitcoin: 'antibitcoin',
  anti: 'antibitcoin',
  cypher: 'cypher',
  cyp: 'cypher',
  unitus: 'unitus',
  uis: 'unitus',
  revolvercoin: 'revolvercoin',
  xre: 'revolvercoin',
  rootcoin: 'rootcoin',
  root: 'rootcoin',
  aerocoin: 'aerocoin',
  am: 'aerocoin',
  arbit: 'arbit',
  arb: 'arbit',
  swing: 'swing',
  chesscoin: 'chesscoin',
  chess: 'chesscoin',
  posex: 'posex',
  pex: 'posex',
  'high-voltage': 'high-voltage',
  hvco: 'high-voltage',
  revenu: 'revenu',
  rev: 'revenu',
  'rhinocoin-rhc': 'rhinocoin-rhc',
  rhc: 'rhinocoin-rhc',
  songcoin: 'songcoin',
  song: 'songcoin',
  amsterdamcoin: 'amsterdamcoin',
  ams: 'amsterdamcoin',
  coexistcoin: 'coexistcoin',
  coxst: 'coexistcoin',
  islacoin: 'islacoin',
  isl: 'islacoin',
  cabbage: 'cabbage',
  cab: 'cabbage',
  helleniccoin: 'helleniccoin',
  hnc: 'helleniccoin',
  gamerholiccoin: 'gamerholiccoin',
  ghc: 'gamerholiccoin',
  pospro: 'pospro',
  psp: 'pospro',
  litecred: 'litecred',
  ltcr: 'litecred',
  vcoin: 'virtualcoin',
  vcn: 'vcoin',
  powercoin: 'powercoin',
  pwr: 'powercoin',
  berncash: 'berncash',
  bern: 'berncash',
  bantam: 'bantam',
  bnt: 'bantam',
  prototanium: 'prototanium',
  pr: 'prototanium',
  ivugeocoin: 'ivugeocoin',
  iec: 'ivugeocoin',
  biteur: 'biteur',
  sydpak: 'sydpak',
  sdp: 'sydpak',
  bitquark: 'bitquark',
  btq: 'bitquark',
  mastertradercoin: 'mastertradercoin',
  mtr: 'mastertradercoin',
  cryptbit: 'cryptbit',
  cbit: 'cryptbit',
  khancoin: 'khancoin',
  knc: 'khancoin',
  trmb: 'trmb',
  blackjack: 'blackjack',
  jack: 'blackjack',
  kilocoin: 'kilocoin',
  klc: 'kilocoin',
  rimbit: 'rimbit',
  rbt: 'rimbit',
  ion: 'ion',
  asiacoin: 'asiacoin',
  ac: 'asiacoin',
  jinn: 'jinn',
  ixcoin: 'ixcoin',
  ixc: 'ixcoin',
  stabilityshares: 'stabilityshares',
  xsi: 'stabilityshares',
  '007coin': '007coin',
  '007': '007coin',
  freemarket: 'freemarket',
  mrkt: 'freemarket',
  kobocoin: 'kobocoin',
  kobo: 'kobocoin',
  nxtventure: 'nxtventure',
  nxtv: 'nxtventure',
  librexcoin: 'librexcoin',
  lxc: 'librexcoin',
  cryptokenz: 'cryptokenz',
  cyt: 'cryptokenz',
  greencoin: 'greencoin',
  gre: 'greencoin',
  eccoin: 'eccoin',
  ecc: 'eccoin',
  btctalkcoin: 'btctalkcoin',
  talk: 'btctalkcoin',
  microcoin: 'microcoin',
  mrc: 'microcoin',
  czechcrowncoin: 'czechcrowncoin',
  czc: 'czechcrowncoin',
  bitstar: 'bitstar',
  bits: 'bitstar',
  ultracoin: 'ultracoin',
  utc: 'ultracoin',
  leafcoin: 'leafcoin',
  leaf: 'leafcoin',
  core: 'core',
  tilecoin: 'tilecoin',
  xtc: 'tilecoin',
  stress: 'stress',
  sts: 'stress',
  debune: 'debune',
  xiaomicoin: 'xiaomicoin',
  mi: 'xiaomicoin',
  coinoindex: 'coinoindex',
  index: 'coinoindex',
  'buongiorno-caffe': 'buongiorno-caffe',
  bgcaffe: 'buongiorno-caffe',
  woodshares: 'woodshares',
  ws: 'woodshares',
  yacoin: 'yacoin',
  yac: 'yacoin',
  sphere: 'sphere',
  sphr: 'sphere',
  communitycoin: 'communitycoin',
  comm: 'communitycoin',
  extremecoin: 'extremecoin',
  exc: 'extremecoin',
  martexcoin: 'martexcoin',
  mxt: 'martexcoin',
  freshcoin: 'freshcoin',
  frsh: 'freshcoin',
  cannacoin: 'cannacoin',
  ccn: 'cannacoin',
  luckycoin: 'luckycoin',
  lky: 'luckycoin',
  fudcoin: 'fudcoin',
  dark: 'fudcoin',
  positron: 'positron',
  tron: 'positron',
  genesyscoin: 'genesyscoin',
  gsy: 'genesyscoin',
  lottocoin: 'lottocoin',
  lot: 'lottocoin',
  newbium: 'newbium',
  newb: 'newbium',
  privatebet: 'privatebet',
  dashcoin: 'dashcoin',
  dsh: 'dashcoin',
  mmbtcd: 'mmbtcd',
  coin: 'coin-coin',
  cno: 'coin',
  checkcoin: 'checkcoin',
  ckc: 'checkcoin',
  incakoin: 'incakoin',
  nka: 'incakoin',
  trollcoin: 'trollcoin',
  troll: 'trollcoin',
  darktron: 'darktron',
  drkt: 'darktron',
  quotient: 'quotient',
  xqn: 'quotient',
  sterlingcoin: 'sterlingcoin',
  slg: 'sterlingcoin',
  datacoin: 'datacoin',
  dtc: 'datacoin',
  particle: 'particle',
  prt: 'particle',
  bigup: 'bigup',
  cryptocircuits: 'cryptocircuits',
  circ: 'cryptocircuits',
  animecoin: 'animecoin',
  ani: 'animecoin',
  node: 'node',
  genstake: 'genstake',
  g3n: 'genstake',
  triangles: 'triangles',
  tri: 'triangles',
  gaia: 'gaia',
  techcoin: 'techcoin',
  tech: 'techcoin',
  'gamebet-coin': 'gamebet-coin',
  gbt: 'gamebet-coin',
  clearinghouse: 'clearinghouse',
  xch: 'clearinghouse',
  bitcointx: 'bitcointx',
  btx: 'bitcointx',
  bottlecaps: 'bottlecaps',
  cap: 'bottlecaps',
  sproutsextreme: 'sproutsextreme',
  spex: 'sproutsextreme',
  saffroncoin: 'saffroncoin',
  sfr: 'saffroncoin',
  aricoin: 'aricoin',
  ari: 'aricoin',
  nyancoin: 'nyancoin',
  nyan: 'nyancoin',
  darkcash: 'darkcash',
  drkc: 'darkcash',
  dobbscoin: 'dobbscoin',
  bob: 'dobbscoin',
  ratecoin: 'ratecoin',
  xra: 'ratecoin',
  hobonickels: 'hobonickels',
  hbn: 'hobonickels',
  platinumbar: 'platinumbar',
  xptx: 'platinumbar',
  sixeleven: 'sixeleven',
  moneta: 'moneta2',
  mnta: 'moneta',
  shacoin: 'shacoin',
  sha: 'shacoin',
  klondikecoin: 'klondikecoin',
  kdc: 'klondikecoin',
  tittiecoin: 'tittiecoin',
  ttc: 'tittiecoin',
  halcyon: 'halcyon',
  hal: 'halcyon',
  kuwaitcoin: 'kuwaitcoin',
  kuc: 'kuwaitcoin',
  globalcoin: 'globalcoin',
  glc: 'globalcoin',
  crowncoin: 'crowncoin',
  crw: 'crowncoin',
  bitgold: 'bitgold',
  satoshimadness: 'satoshimadness',
  mad: 'satoshimadness',
  returncoin: 'returncoin',
  rnc: 'returncoin',
  octocoin: 'octocoin',
  russiacoin: 'russiacoin',
  rc: 'russiacoin',
  swagbucks: 'swagbucks',
  bucks: 'swagbucks',
  o2olondoncoin: 'o2olondoncoin',
  '020': 'o2olondoncoin',
  litedoge: 'litedoge',
  ldoge: 'litedoge',
  spacecoin: 'spacecoin',
  space: 'spacecoin',
  'x-coin': 'x-coin',
  xco: 'x-coin',
  petrodollar: 'petrodollar',
  xpd: 'petrodollar',
  neocoin: 'neocoin',
  nec: 'neocoin',
  mapcoin: 'mapcoin',
  mapc: 'mapcoin',
  bitbar: 'bitbar',
  btb: 'bitbar',
  destiny: 'destiny',
  des: 'destiny',
  mgw: 'mgw',
  ringo: 'ringo',
  rin: 'ringo',
  orlycoin: 'orlycoin',
  orly: 'orlycoin',
  fastcoin: 'fastcoin',
  fst: 'fastcoin',
  fujicoin: 'fujicoin',
  fjc: 'fujicoin',
  htmlcoin: 'htmlcoin',
  html5: 'htmlcoin',
  emerald: 'emerald',
  emd: 'emerald',
  granitecoin: 'granitecoin',
  grn: 'granitecoin',
  bunnycoin: 'bunnycoin',
  bun: 'bunnycoin',
  pulse: 'pulse',
  graffiti: 'graffiti',
  grf: 'graffiti',
  'bdsm-fetish': 'bdsm-fetish',
  whipped: 'bdsm-fetish',
  'bitcoin-scrypt': 'bitcoin-scrypt',
  btcs: 'bitcoin-scrypt',
  bitzeny: 'bitzeny',
  zny: 'bitzeny',
  captcoin: 'captcoin',
  capt: 'captcoin',
  elcoin: 'elcoin',
  elco: 'elcoin',
  phoenixcoin: 'phoenixcoin',
  pxc: 'phoenixcoin',
  goldreserve: 'goldreserve',
  xgr: 'goldreserve',
  argentum: 'argentum',
  arg: 'argentum',
  kumacoin: 'kumacoin',
  kuma: 'kumacoin',
  jay: 'jay',
  spt: 'spots',
  unicoin: 'unicoin',
  unic: 'unicoin',
  joulecoin: 'joulecoin',
  xjo: 'joulecoin',
  trapcoin: 'trapcoin',
  trap: 'trapcoin',
  tigercoin: 'tigercoin',
  tgc: 'tigercoin',
  guncoin: 'guncoin',
  gun: 'guncoin',
  'philosopher-stones': 'philosopher-stones',
  phs: 'philosopher-stones',
  limitedcoin: 'limitedcoin',
  ltd: 'limitedcoin',
  hamradiocoin: 'hamradiocoin',
  ham: 'hamradiocoin',
  'metal-music-coin': 'metal-music-coin',
  mtlmc3: 'metal-music-coin',
  popularcoin: 'popularcoin',
  pop: 'popularcoin',
  gcoin: 'gcoin',
  gcn: 'gcoin',
  breakcoin: 'breakcoin',
  bre: 'breakcoin',
  wmcoin: 'wmcoin',
  wmc: 'wmcoin',
  cybercoin: 'cybercoin',
  cc: 'cybercoin',
  topcoin: 'topcoin',
  top: 'topcoin',
  cagecoin: 'cagecoin',
  cage: 'cagecoin',
  digitalprice: 'digitalprice',
  dp: 'digitalprice',
  mangocoinz: 'mangocoinz',
  mcz: 'mangocoinz',
  evotion: 'evotion',
  evo: 'evotion',
  acoin: 'acoin',
  'vip-tokens': 'vip-tokens',
  vip: 'vip-tokens',
  anarchistsprime: 'anarchistsprime',
  acp: 'anarchistsprime',
  bowscoin: 'bowscoin',
  bsc: 'bowscoin',
  'independent-money-system': 'independent-money-system',
  ims: 'independent-money-system',
  '42-coin': '42-coin',
  secfund: 'secfund',
  tagrcoin: 'tagrcoin',
  tagr: 'tagrcoin',
  bolivarcoin: 'bolivarcoin',
  boli: 'bolivarcoin',
  mazecoin: 'mazecoin',
  maze: 'mazecoin',
  axiom: 'axiom',
  icash: 'icash',
  parallelcoin: 'parallelcoin',
  duo: 'parallelcoin',
  nxtinspect: 'nxtinspect',
  nxti: 'nxtinspect',
  phalanx: 'phalanx',
  pxl: 'phalanx',
  roscoin: 'roscoin',
  ros: 'roscoin',
  mmxvi: 'mmxvi',
  newyorkcoin: 'newyorkcoin',
  nyc: 'newyorkcoin',
  dreamcoin: 'dreamcoin',
  drm: 'dreamcoin',
  bloodcoin: 'bloodcoin',
  bod: 'bloodcoin',
  ronpaulcoin: 'ronpaulcoin',
  rpc: 'ronpaulcoin',
  chaincoin: 'chaincoin',
  chc: 'chaincoin',
  vibranium: 'vibranium',
  xvi: 'vibranium',
  zurcoin: 'zurcoin',
  zur: 'zurcoin',
  guarany: 'guarany',
  gua: 'guarany',
  beavercoin: 'beavercoin',
  bvc: 'beavercoin',
  zombiecoin: 'zombiecoin',
  zmc: 'zombiecoin',
  flavorcoin: 'flavorcoin',
  '2flav': 'flavorcoin',
  bitcrystal: 'bitcrystal',
  btcry: 'bitcrystal',
  guccionecoin: 'guccionecoin',
  gcc: 'guccionecoin',
  fuzzballs: 'fuzzballs',
  fuzz: 'fuzzballs',
  freedomcoin: 'freedomcoin',
  frdc: 'freedomcoin',
  machinecoin: 'machinecoin',
  mac: 'machinecoin',
  coinaid: 'coinaid',
  caid: 'coinaid',
  unrealcoin: 'unrealcoin',
  urc: 'unrealcoin',
  nevacoin: 'nevacoin',
  neva: 'nevacoin',
  wearesatoshi: 'wearesatoshi',
  wsx: 'wearesatoshi',
  leacoin: 'leacoin',
  lea: 'leacoin',
  floz: 'floz',
  unfed: 'unfed',
  unf: 'unfed',
  cygnus: 'cygnus',
  cyg: 'cygnus',
  litebar: 'litebar',
  ltb: 'litebar',
  ozziecoin: 'ozziecoin',
  ozc: 'ozziecoin',
  lanacoin: 'lanacoin',
  lana: 'lanacoin',
  alexium: 'alexium',
  aum: 'alexium',
  tennet: 'tennet',
  lyrabar: 'lyrabar',
  lyb: 'lyrabar',
  'bitcoin-21': 'bitcoin-21',
  xbtc21: 'bitcoin-21',
  fistbump: 'fistbump',
  fist: 'fistbump',
  cryptospots: 'cryptospots',
  cs: 'cryptospots',
  virtualcoin: 'virtualcoin',
  clevercoin: 'clevercoin',
  clv: 'clevercoin',
  heelcoin: 'heelcoin',
  heel: 'heelcoin',
  selfiecoin: 'selfiecoin',
  slfi: 'selfiecoin',
  subcriptio: 'subcriptio',
  sub: 'subcriptio',
  'save-and-gain': 'save-and-gain',
  sandg: 'save-and-gain',
  osmiumcoin: 'osmiumcoin',
  os76: 'osmiumcoin',
  cashcoin: 'cashcoin',
  cash: 'cashcoin',
  palcoin: 'palcoin',
  pal: 'palcoin',
  antilitecoin: 'antilitecoin',
  altc: 'antilitecoin',
  ponzicoin: 'ponzicoin',
  ponzi: 'ponzicoin',
  hazmatcoin: 'hazmatcoin',
  hzt: 'hazmatcoin',
  'evil-coin': 'evil-coin',
  evil: 'evil-coin',
  batcoin: 'batcoin',
  eurocoin: 'eurocoin',
  euc: 'eurocoin',
  growcoin: 'growcoin',
  grow: 'growcoin',
  stronghands: 'stronghands',
  shnd: 'stronghands',
  number7: 'number7',
  n7: 'number7',
  crypto: 'crypto',
  cto: 'crypto',
  money: 'money',
  $$$: 'money',
  zoom: 'zoom',
  p7coin: 'p7coin',
  p7c: 'p7coin',
  turbostake: 'turbostake',
  trbo: 'turbostake',
  cerium: 'cerium',
  xce: 'cerium',
  donationcoin: 'donationcoin',
  don: 'donationcoin',
  brexitcoin: 'brexitcoin',
  brxt: 'brexitcoin',
  legendarycoin: 'legendarycoin',
  lgd: 'legendarycoin',
  crtcoin: 'crtcoin',
  crt: 'crtcoin',
  imperialcoin: 'imperialcoin',
  ipc: 'imperialcoin',
  hundredcoin: 'hundredcoin',
  hun: 'hundredcoin',
  californium: 'californium',
  cf: 'californium',
  enigma: 'enigma',
  xng: 'enigma',
  'digital-credits': 'digital-credits',
  dgcs: 'digital-credits',
  'coin-coin': 'coin-coin',
  '23-skidoo': '23-skidoo',
  chao: '23-skidoo',
  xdecoin: 'xdecoin',
  xde: 'xdecoin',
  horiemoncard: 'horiemoncard',
  hor: 'horiemoncard',
  crycoin: 'crycoin',
  cry: 'crycoin',
  forevercoin: 'forevercoin',
  xfc: 'forevercoin',
  hommalicoin: 'hommalicoin',
  hmc: 'hommalicoin',
  duckduckcoin: 'duckduckcoin',
  duck: 'duckduckcoin',
  swiscoin: 'swiscoin',
  scn: 'swiscoin',
  nxttyacci: 'nxttyacci',
  acci: 'nxttyacci',
  edrcoin: 'edrcoin',
  edrc: 'edrcoin',
  cryptobyte: 'cryptobyte',
  cb: 'cryptobyte',
  'steem-dollars': 'steem-dollars',
  sbd: 'steem-dollars',
  coinousd: 'coinousd',
  xusd: 'coinousd',
  leocoin: 'leocoin',
  leo: 'leocoin',
  'ico-openledger': 'ico-openledger',
  icoo: 'ico-openledger',
  npccoin: 'npccoin',
  npc: 'npccoin',
  'breakout-stake': 'breakout-stake',
  brx: 'breakout-stake',
  mayacoin: 'mayacoin',
  myc: 'mayacoin',
  futurepoints: 'futurepoints',
  ftp: 'futurepoints',
  clubcoin: 'clubcoin',
  club: 'clubcoin',
  invisiblecoin: 'invisiblecoin',
  ivz: 'invisiblecoin',
  bitstake: 'bitstake',
  xbs: 'bitstake',
  'asset-backed-coin': 'asset-backed-coin',
  abc: 'asset-backed-coin',
  pabyosicoin: 'pabyosicoin',
  pbc: 'pabyosicoin',
  bitland: 'bitland',
  cadastral: 'bitland',
  uncoin: 'uncoin',
  'heat-ledger': 'heat-ledger',
  heat: 'heat-ledger',
  alpacoin: 'alpacoin',
  'bnb-coin': 'bnb-coin',
  bnb: 'bnb-coin',
  gbcgoldcoin: 'gbcgoldcoin',
  gbc: 'gbcgoldcoin',
  firecoin: 'firecoin',
  fire: 'firecoin',
  nocturna: 'nocturna',
  noc: 'nocturna',
  megastake: 'megastake',
  xms: 'megastake',
  'international-diamond': 'international-diamond',
  xid: 'international-diamond',
  'alphabet-coin-fund': 'alphabet-coin-fund',
  fun: 'alphabet-coin-fund',
  bitseeds: 'bitseeds',
  xseed: 'bitseeds',
  faucetcoin: 'faucetcoin',
  drop: 'faucetcoin',
  stakerush: 'stakerush',
  sthr: 'stakerush',
  tbcoin: 'tbcoin',
  tbc: 'tbcoin',
  teslacoilcoin: 'teslacoilcoin',
  tesla: 'teslacoilcoin',
  resumeoshares: 'resumeoshares',
  rms: 'resumeoshares',
  omega: 'omega',
  oma: 'omega',
  goldmaxcoin: 'goldmaxcoin',
  gmx: 'goldmaxcoin',
  bitmoon: 'bitmoon',
  bm: 'bitmoon',
  timekoin: 'timekoin',
  tk: 'timekoin',
  sharkcoin: 'sharkcoin',
  sak: 'sharkcoin',
  bitalphacoin: 'bitalphacoin',
  bac: 'bitalphacoin',
  'project-decorum': 'project-decorum',
  pdc: 'project-decorum',
  graviton: 'graviton',
  grav: 'graviton',
  spikesprivatecoin: 'spikesprivatecoin',
  spc: 'spikesprivatecoin',
  vtorrent: 'vtorrent',
  vtr: 'vtorrent',
  skullbuzz: 'skullbuzz',
  skb: 'skullbuzz',
  enecoin: 'enecoin',
  ene: 'enecoin',
  roxcoin: 'roxcoin',
  rox: 'roxcoin',
  'asian-investment-bond': 'asian-investment-bond',
  aib: 'asian-investment-bond',
  'happy-creator-coin': 'happy-creator-coin',
  hcc: 'happy-creator-coin',
  bagcoin: 'bagcoin',
  bgc: 'bagcoin',
  safecoin: 'safecoin',
  sfe: 'safecoin',
  shellpay: 'shellpay',
  shell: 'shellpay',
  taopay: 'taopay',
  tao: 'taopay',
  eclipse: 'eclipse',
  ec: 'eclipse',
  kcoin: 'kcoin',
  kc: 'kcoin',
  clinton: 'clinton',
  clint: 'clinton',
  gotfomo: 'gotfomo',
  gtfo: 'gotfomo',
  xaucoin: 'xaucoin',
  eggcoin: 'eggcoin',
  egg: 'eggcoin',
  vector: 'vector',
  vec: 'vector',
  bitcedi: 'bitcedi',
  bxc: 'bitcedi',
  incrementum: 'incrementum',
  inc: 'incrementum',
  futcoin: 'futcoin',
  futc: 'futcoin',
  'cthulhu-offerings': 'cthulhu-offerings',
  off: 'cthulhu-offerings',
  richcoin: 'richcoin',
  richx: 'richcoin',
  profitcoin: 'profitcoin',
  pfc: 'profitcoin',
  chncoin: 'chncoin',
  cnc: 'chncoin',
  eggs: 'eggs',
  c0fferc0in: 'c0fferc0in',
  c0c0: 'c0fferc0in',
  nxe: 'nxe',
  dotcoin: 'dotcoin',
  dot: 'dotcoin',
  rublebit: 'rublebit',
  rubit: 'rublebit',
  irishcoin: 'irishcoin',
  irl: 'irishcoin',
  friendshipcoin2: 'friendshipcoin2',
  fsc2: 'friendshipcoin2',
  upcoin: 'upcoin',
  xup: 'upcoin',
  'local-family-owned': 'local-family-owned',
  lfo: 'local-family-owned',
  bumbacoin: 'bumbacoin',
  bumba: 'bumbacoin',
  psilocybin: 'psilocybin',
  psy: 'psilocybin',
  trickycoin: 'trickycoin',
  trick: 'trickycoin',
  asiccoin: 'asiccoin',
  asc: 'asiccoin',
  grexit: 'grexit',
  ocow: 'ocow',
  chronos: 'chronos',
  crx: 'chronos',
  swaptoken: 'swaptoken',
  token: 'swaptoken',
  bithire: 'bithire',
  hire: 'bithire',
  gsmcoin: 'gsmcoin',
  gsm: 'gsmcoin',
  digieuro: 'digieuro',
  deur: 'digieuro',
  quebecoin: 'quebecoin',
  qbc: 'quebecoin',
  dubstep: 'dubstep',
  dub: 'dubstep',
  bitcoinfast: 'bitcoinfast',
  bcf: 'bitcoinfast',
  linkedcoin: 'linkedcoin',
  lkc: 'linkedcoin',
  worldpay: 'worldpay',
  wop: 'worldpay',
  darkether: 'darkether',
  deth: 'darkether',
  'digital-bullion-gold': 'digital-bullion-gold',
  dbg: 'digital-bullion-gold',
  operand: 'operand',
  op: 'operand',
  pentacoin: 'pentacoin',
  pta: 'pentacoin',
  murraycoin: 'murraycoin',
  mry: 'murraycoin',
  diggits: 'diggits',
  digs: 'diggits',
  motocoin: 'motocoin',
  moto: 'motocoin',
  litestarcoin: 'litestarcoin',
  lts: 'litestarcoin',
  gridpay: 'gridpay',
  grid: 'gridpay',
  avatarcoin: 'avatarcoin',
  av: 'avatarcoin',
  braincoin: 'braincoin',
  brain: 'braincoin',
  prismchain: 'prismchain',
  prm: 'prismchain',
  skeincoin: 'skeincoin',
  skc: 'skeincoin',
  ibits: 'ibits',
  darklisk: 'darklisk',
  disk: 'darklisk',
  supreme: 'supreme',
  spm: 'supreme',
  candlecoin: 'candlecoin',
  cd: 'candlecoin',
  opescoin: 'opescoin',
  opes: 'opescoin',
  flaxscript: 'flaxscript',
  flax: 'flaxscript',
  vegascoin: 'vegascoin',
  vgc: 'vegascoin',
  darkcypher: 'darkcypher',
  dcyp: 'darkcypher',
  thecreed: 'thecreed',
  tcr: 'thecreed',
  moneta2: 'moneta2',
  '2bacco': '2bacco',
  biebercoin: 'biebercoin',
  brc: 'biebercoin',
  valorbit: 'valorbit',
  val: 'valorbit',
  soulcoin: 'soulcoin',
  soul: 'soulcoin',
  superturbostake: 'superturbostake',
  strb: 'superturbostake',
  purepos: 'purepos',
  pure: 'purepos',
  stakers: 'stakers',
  sta: 'stakers',
  '404-coin': '404-coin',
  fireflycoin: 'fireflycoin',
  ffc: 'fireflycoin',
  cashme: 'cashme',
  cme: 'cashme',
  x2: 'x2',
  paypeer: 'paypeer',
  payp: 'paypeer',
  trinity: 'trinity',
  tty: 'trinity',
  espers: 'espers',
  esp: 'espers',
  paccoin: 'paccoin',
  pac: 'paccoin'
}

export default crypto
